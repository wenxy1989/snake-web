...
<form id="frm" action="${"$"}{basePath }/${module?uncap_first}/create" method="POST">
<#list attributes as attribute>
<div class="field">
<#if attribute.pageStyle == "text">
...
<div id="div_${attribute.code?uncap_first}" class="ui selection dropdown">
  ...
  <div class="text" data-value="${"$"}{object.${attribute.code?uncap_first} }">${"$"}{object.${attribute.code?uncap_first} }</div>
...
<input type="checkbox" name="${attribute.code?uncap_first}" value="1"<c:if test="${"$"}{object.${attribute.code?uncap_first} == 1}"> checked="checked"</c:if>>
...
<div class="field">
<div class="ui submit blue button" >Create</div>
