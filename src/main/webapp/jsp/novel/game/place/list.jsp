...
<body>
            ...
            <c:forEach var="each" items="${objectList }" varStatus="s">
            	<li title="${each.createTime }"<c:if test="${s.last }">class="unborder"</c:if>>
            		...
            		<span class="right">
            			...
            			<span><a href="${basePath }/${e.path }?characterId=${each.id }" target="_blank">${e.name }</a></span>
            			</c:forEach>
            			<span><a href="javascript:doDelete(${each.id },${each.novalId })">删除</a></span>
            			<span><a href="${basePath }/character/toEdit.do?id=${each.id }${each.urlBelong }">编辑</a></span>
            		</span>
            	</li>
            </c:forEach>
            </ul>
    ...
    <div class="i_c">
        <div class="i_cont">
        	<ul class="r_con03">
        	<li><a href="${basePath }/character/toCreate.do?${object.urlBelong }">新建</a>	</li>
			</ul>
		</div>
	</div>
</body>
