...
function doDelete(id,novelId){
	if(confirm("确认删除？")){
		window.location.href = "${basePath }/world/delete.do?id=" + id + "&novelId="+novelId;
	}
}
</script>
</head>
<body>
        ...
        <div class="i_cont">
            ...
            <c:forEach var="each" items="${objectList }" varStatus="s">
            	<li title="${each.createTime }"<c:if test="${s.last }">class="unborder"</c:if>>
            		<span class="left"><a href="${basePath }/world/details.do?id=${each.id }">${each.name }</a></span>
            		<span class="right">
            			<span>${each.title }</span>
            			<span><a href="${basePath }/element/list.do?belongId=${each.id }" target="_blank">要素</a></span>
            			<c:forEach items="${each.elements }" var="e">
            			<span><a href="${basePath }/${e.path }?worldId=${each.id }" target="_blank">${e.name }</a></span>
            			</c:forEach>
            			<span><a href="javascript:doDelete(${each.id },${each.novelId })">删除</a></span>
            			<span><a href="${basePath }/world/toEdit.do?id=${each.id }">编辑</a></span>
            		</span>
            	</li>
            </c:forEach>
            </ul>
    ...
    <div class="i_c">
        <div class="i_cont">
        	<ul class="r_con03">
        	<li><a href="${basePath }/world/toCreate.do?${object.urlBelong }">新建</a></li>
			</ul>
		</div>
	</div>
</body>
