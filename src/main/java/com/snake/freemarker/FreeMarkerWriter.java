...
public class FreeMarkerWriter {
    ...
    public FreeMarkerWriter(String folder) throws IOException {
        this.init(folder);
    }

    public FreeMarkerWriter() throws IOException {
        this.init(null);
    }
    ...
    private void init(String folder) throws IOException {
        folder = StringUtils.isNotBlank(folder) ? folder : getClassPath() + "/template/";
        if (null == this.templateConfiguration) {
            this.templateConfiguration = new Configuration();
            this.templateConfiguration.setDirectoryForTemplateLoading(new File(folder));
        }
        if (null == outputPath) {
            String webPath = new File(getClassPath()).getParentFile().getParent();
            this.outputPath = webPath + "/output";
        }
    }

    private Template getTemplate(TemplateConfig config) throws IOException {
        String path = config.getGroup() + "/" + config.getName();
        path = path.replaceAll("//", "/");
        return this.templateConfiguration.getTemplate(path, SYSTEM_ENCODING);
    }
