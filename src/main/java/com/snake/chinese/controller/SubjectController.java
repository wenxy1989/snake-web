package com.snake.chinese.controller;

import com.base.Constants;
import com.snake.system.controller.BasicController;
import com.base.exception.ServiceException;
import com.base.util.Condition;
import com.base.util.Criteria;
import com.base.util.DateTimeUtils;
import com.base.util.SimpleCriteria;
import com.snake.chinese.model.relation.Subject;
import com.snake.chinese.service.ISubjectService;
import org.apache.commons.lang.StringUtils;
import org.springframework.data.domain.Page;
import org.springframework.stereotype.Controller;
import org.springframework.web.bind.annotation.RequestMapping;
import org.springframework.web.servlet.ModelAndView;
import org.springframework.web.servlet.view.RedirectView;

import javax.annotation.Resource;
import javax.servlet.http.HttpServletRequest;

/**
 * Created by wen on 2015/5/1.
 */
@Controller
@RequestMapping("/book/subject/")
public class SubjectController extends BasicController {

    @Resource(name = "subjectService")
    private ISubjectService subjectService;

    @RequestMapping(value = "page")
    public ModelAndView page(Integer pageSize,Integer pageNo,HttpServletRequest request){
        ModelAndView mv = new ModelAndView("/chinese/subject/list");
        String name = request.getParameter("name");
        String subject = request.getParameter("subject");
        Criteria c = new SimpleCriteria();
        if(StringUtils.isNotEmpty(name)){
            c.addCondition(0,new Condition("name_","like","%"+name+"%"));
            mv.addObject("name",name);
        }
        if(StringUtils.isNotEmpty(subject)){
            c.addCondition(0,new Condition("subject_","=",subject));
            mv.addObject("subject", subject);
        }
        try {
            c.setPageNo(pageNo == null?1:pageNo);
            c.setFetchSize(pageSize == null?Constants.PAGE_SIZE:pageSize);
            Page<Subject> page = subjectService.getList(c);
            mv.addObject("page",page);
        }catch (ServiceException e){
            logger.error("获取主语实体信息失败",e);
        }
        return mv;
    }

    @RequestMapping(value = "toAdd")
    public ModelAndView toAdd(){
        ModelAndView mv = new ModelAndView("/chinese/subject/add");
        return mv;
    }

    @RequestMapping(value = "add")
    public RedirectView add(Subject subject){
        RedirectView rv = new RedirectView("page");
        try{
            subject.setCreatedTime(DateTimeUtils.getNowDateTime());
            subjectService.create(subject);
        }catch (ServiceException e){
            logger.error("add book word subject failed",e);
        }
        return rv;
    }

    @RequestMapping(value = "toEdit")
    public ModelAndView toEdit(Long id){
        ModelAndView mv = new ModelAndView("/chinese/subject/edit");
        try {
            Subject subject = subjectService.getObject(id);
            mv.addObject("obj", subject);
        }catch (ServiceException e){
            logger.error("get subject failed",e);
        }
        return mv;
    }

    @RequestMapping(value = "edit")
    public RedirectView edit(Subject subject){
        RedirectView rv = new RedirectView("page");
        try{
            subjectService.update(subject);
        }catch (ServiceException e){
            logger.error("edit book word subject failed",e);
        }
        return rv;
    }

    @RequestMapping(value = "delete")
    public RedirectView delete(Long id){
        RedirectView rv = new RedirectView("page");
        try{
            subjectService.delete(id);
        }catch (ServiceException e){
            logger.error("add book word subject failed",e);
        }
        return rv;
    }

}
